...
def main(params_override: Optional[Dict[str, Any]] = None) -> SimRunResult:
    ...
    # --- Output directory and logging setup (moved inside) ---
    OUTDIR = _make_run_dir(prefix="runs")
    ...
    while t < t_end - 1e-12:
        ...
        if need_rebuild:
            _a_w, _cprime_w, _inv_w = _build_cn_factors(Nx, lam_w, p["theta_cond"], DTYPE)
            _a_i, _cprime_i, _inv_i = _build_cn_factors(Nx, lam_i, p["theta_cond"], DTYPE)
            _dt_last = dt
...
if __name__ == "__main__":
    # CLI / direct execution path (uses defaults)
    _ = main()