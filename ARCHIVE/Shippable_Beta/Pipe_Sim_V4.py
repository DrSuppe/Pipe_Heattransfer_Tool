...
def _build_thermal_mass_profile(nx: int, count: int, factor: float, positions_frac: list[float] | np.ndarray, spread_frac: float):
    ...
    for frac in pos:
        ...
        for dj in range(-3 * spread, 3 * spread + 1):
            ...
            if j < 0 or j >= nx:
                continue
            w = max(0.0, 1.0 - abs(dj) / (3.0 * spread + 1.0e-12))
...
def _h_out_natural_conv_scalar(Ts, Tamb, D_out):
    ...
    Ra = g * beta * dT * (D ** 3) / (nu * alpha + 1.0e-20)
    if Ra < 1.0e-14:
        return 1.0
    Nu = (0.60 + (0.387 * (Ra ** (1.0 / 6.0))) / ((1.0 + (0.559 / Pr_air) ** (9.0 / 16.0)) ** (8.0 / 27.0))) ** 2
    h = Nu * k_air / D
    if h < 1.0:
        h = 1.0
    return h
...
def _h_out_natural_conv_vec(Ts_vec, Tamb, D_out):
    ...
    Ra = g * beta * dT * (D ** 3) / (nu * alpha + 1.0e-20)
    Nu = (0.60 + (0.387 * np.power(np.maximum(Ra, 1.0e-14), 1.0 / 6.0)) / ((1.0 + (0.559 / Pr_air) ** (9.0 / 16.0)) ** (8.0 / 27.0))) ** 2
    h = Nu * k_air / D
...
def _timestep_numba_seq(Tg, Tw, Ti, dt,
    ...
    for i in range(n):
        ...
        a11 = Cw_cell_vec[i]/dt + h_in[i]*P_in + inv_R
        a12 = -inv_R
        a21 = -inv_R
        a22 = Ci_cell_vec[i]/dt + inv_R + h_out_i*P_out + k_rad
        rhs1 = (Cw_cell_vec[i]/dt)*Tw[i] + h_in[i]*P_in*Tg_out[i]
        rhs2 = (Ci_cell_vec[i]/dt)*Ti[i] + h_out_i*P_out*Tamb + eps_rad*sigma*P_out*((Tamb**4) - (Ti[i]**4)) + k_rad*Ti[i]
        det = a11*a22 - a12*a21
...
def _timestep_numba(Tg, Tw, Ti, dt,
    ...
    for i in prange(n):
        ...
        a21 = -inv_R
        a22 = Ci_cell_vec[i]/dt + inv_R + h_out_i*P_out + k_rad
        rhs1 = (Cw_cell_vec[i]/dt)*Tw[i] + h_in[i]*P_in*Tg_new[i]
        rhs2 = (Ci_cell_vec[i]/dt)*Ti[i] + h_out_i*P_out*Tamb + eps_rad*sigma*P_out*((Tamb**4) - (Ti[i]**4)) + k_rad*Ti[i]
        det = a11*a22 - a12*a21
        Tw_e[i] = (rhs1*a22 - a12*rhs2)/det
        Ti_e[i] = (a11*rhs2 - rhs1*a21)/det
...
def main(
    ...
    def build_solid_state(Tw_vec, Ti_vec):
        ...
        Ci_vec = (p["rho_i"] * Vi_cell * cp_i_vec).astype(DTYPE, copy=False)
    ...
    tau_wi = float(np.min(Cw_cell_vec * np.maximum(R_Tw_to_Ti_vec, 1.0e-16)))
    ...
    while t < t_end - 1e-12:
        ...
        tau_w_local = Cw_cell_vec/( (1.0/np.maximum(R_Tw_to_Ti_vec, 1.0e-16)) + 1e-12 )
        if use_auto_hout == 1:
            h_out_local = _h_out_natural_conv_vec(Ti, p["Tamb"], float(D_out))
        else:
            h_out_local = h_out_manual
        if float(np.max(Ci_cell_vec)) > 1.0e-12:
            tau_i_local = Ci_cell_vec / (h_out_local * P_out + 4 * p["eps_rad"] * sigma * P_out * np.maximum(Ti, 1) ** 3 + 1e-12)
        else:
        ...
        lam_w = float(max(1.0e-16, aw_eff) * dt / (dx * dx))
        lam_i = float(max(1.0e-16, ai_eff) * dt / (dx * dx))
        ...
        try:
            if p.get("parallel", False):
                Tg_new, Tw_new, Ti_new = _timestep_numba(
                    ...
                    _float(p["p"]), _float(p["m_dot"]), _float(Tin_eff), _float(R), _float(p["Pr"]),
                    _float(p["mu_g"]), _float(p["k_g"]),
                    _float(Di), _float(A_flow), _float(P_in), _float(P_out),
                    _float(p["cp_g"]), Cw_cell_vec, Ci_cell_vec, R_Tw_to_Ti_vec,
                    _float(h_out_manual), use_auto_hout, _float(D_out), _float(p["eps_rad"]), _float(sigma),
            ...
            if p["adv_scheme"] == "semi_lagrangian":
                x_depart = x - u*dt
                Tg_adv = np.interp(x_depart, x, Tg, left=Tin_eff, right=Tg[-1]).astype(DTYPE, copy=False)
                q_gw = h_in*P_in*(Tw - Tg_adv)
                Cg_cell = (p["p"]/(R*Tg))*A_flow*dx*p["cp_g"]
                Tg_new = Tg_adv + dt * ( q_gw / np.maximum(Cg_cell,1e-12) )
            else:
                Tg_up = np.roll(Tg, 1); Tg_up[0] = Tin_eff
                adv = -u*(Tg - Tg_up)/dx
            ...
            for i in range(Nx):
                inv_R = 1.0 / np.maximum(R_Tw_to_Ti_vec[i], 1.0e-16)
                h_out_i = h_out_manual
                if use_auto_hout == 1:
                    h_out_i = _h_out_natural_conv_scalar(float(Ti[i]), float(p["Tamb"]), float(D_out))
                a11 = Cw_cell_vec[i]/dt + h_in[i]*P_in + inv_R
                a12 = -inv_R
                a21 = -inv_R
                a22 = Ci_cell_vec[i]/dt + inv_R + h_out_i*P_out + k_rad_vec[i]
                rhs1 = (Cw_cell_vec[i]/dt)*Tw[i] + h_in[i]*P_in*Tg_new[i]
                rhs2 = (Ci_cell_vec[i]/dt)*Ti[i] + h_out_i*P_out*p["Tamb"] + p["eps_rad"]*sigma*P_out*((p["Tamb"]**4) - (Ti[i]**4)) + k_rad_vec[i]*Ti[i]
                det = a11*a22 - a12*a21
