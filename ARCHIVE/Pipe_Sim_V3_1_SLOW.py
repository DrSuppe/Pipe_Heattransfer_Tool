...
params = {
    ...
    # --- Physics-aware approximation controls ---
    "use_uniform_h": False,  # use uniform h_in and u from Tḡ (piecewise-constant across x)
    "update_props_every": 5,  # refresh h_in/u every K steps (already used for non-uniform too)
    "rad_enabled": False,  # allow disabling radiation entirely
    "rad_update_every": 5,  # refresh cached radiation terms every K steps
    "rad_delta_T_thresh": 1.0,  # [K] recompute radiation terms early if max|ΔTi| exceeds this
...
def _timestep_numba_seq(Tg, Tw, Ti, dt,
    ...
    # Local 2x2 for (Tw, Ti): USE CACHED RADIATION ARRAYS
    inv_R = 1.0 / (R_Tw_to_Ti if R_Tw_to_Ti > 1e-16 else 1e-16)
    for i in range(n):
        k_rad = k_rad_vec[i]  # precomputed 4 ε σ P_out Ti^3 (or 0 if rad disabled)
        a11 = Cw_cell / dt + h_in[i] * P_in + inv_R
        a12 = -inv_R
        a21 = -inv_R
        a22 = Ci_cell / dt + inv_R + h_out * P_out + k_rad
        rhs1 = (Cw_cell / dt) * Tw[i] + h_in[i] * P_in * Tg_out[i]
        rhs2 = (Ci_cell / dt) * Ti[i] + h_out * P_out * Tamb + rad_nl_vec[i] + k_rad * Ti[i]
        det = a11 * a22 - a12 * a21
...
# --- Cached transport properties (h_in, u) ---
_h_in = h0.copy()
_u    = u0.copy()
_update_props_every = int(max(1, params.get("update_props_every", 1)))
_last_props_step = -1

# --- Cached radiation terms ---
_rad_enabled = bool(params.get("rad_enabled", True))
_rad_update_every = int(max(1, params.get("rad_update_every", _update_props_every)))
_rad_delta_T_thresh = float(params.get("rad_delta_T_thresh", 1.0))
_k_rad_vec = np.zeros(Nx, dtype=DTYPE)      # 4 ε σ P_out Ti^3
_rad_nl_vec = np.zeros(Nx, dtype=DTYPE)     # ε σ P_out (Tamb^4 − Ti^4)
_last_rad_step = -1
_Ti_ref_for_rad = Ti.copy()
...
while t < t_end - 1e-12:
    ...
    # --- Update h_in and u either uniformly (from Tḡ) or per-cell, but only every K steps ---
    if (n - _last_props_step) >= _update_props_every:
        if bool(params.get("use_uniform_h", False)):
            Tg_bar = float(Tg.mean())
            rho_bar = params["p"] / (R * Tg_bar)
            u_bar = params["m_dot"] / (rho_bar * A_flow)
            Re_bar = max(rho_bar * u_bar * Di / params["mu_g"], 1.0)
            Nu_bar = 0.023 * (Re_bar ** 0.8) * (params["Pr"] ** params["dittus_boelter_n"])
            h_bar = Nu_bar * params["k_g"] / Di
            _h_in[:] = h_bar
            _u[:] = u_bar
        else:
    ...
    # --- Radiation caching / optional disable ---
    if not _rad_enabled:
        # radiation off
        _k_rad_vec.fill(0.0)
        _rad_nl_vec.fill(0.0)
    else:
        need_rad = (n - _last_rad_step) >= _rad_update_every
        if not need_rad:
            # early refresh if Ti drifted significantly
            dTi_max = float(np.max(np.abs(Ti - _Ti_ref_for_rad)))
            need_rad = dTi_max >= _rad_delta_T_thresh
        if need_rad:
            # k_rad = 4 ε σ P_out Ti^3
            Ti3 = np.maximum(Ti, 1.0) ** 3
            _k_rad_vec[:] = 4.0 * params["eps_rad"] * sigma * P_out * Ti3
            # explicit nonlinear term ε σ P_out (Tamb^4 − Ti^4)
            _rad_nl_vec[:] = params["eps_rad"] * sigma * P_out * ((params["Tamb"] ** 4) - (np.maximum(Ti, 1.0) ** 4))
            _Ti_ref_for_rad[:] = Ti
            _last_rad_step = n
...
# Ensure CSV is float64 for readability
summary = summary.astype(np.float64, copy=False)
