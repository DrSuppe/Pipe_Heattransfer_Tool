...
def _timestep_numba_seq(Tg, Tw, Ti, dt,
    ...
    if use_semi_lag == 1:
        ...
        Tg_adv = _interp1d_uniform_vec(x0, dx, Tg, xp_buf, Tin, Tg[-1])
        Tg_new = np.empty_like(Tg)
        for i in range(n):
            Cg_buf[i]  = rho[i]*A_flow*dx*cp_g
            q_gw_buf[i]= h_in[i]*P_in*(Tw[i] - Tg_adv[i])
            Tg_new[i]  = Tg_adv[i] + dt*(q_gw_buf[i]/max(Cg_buf[i], 1e-12))
    else:
        Tg_new = np.empty_like(Tg)
        for i in range(n):
...
while t < t_end - 1e-12:
    ...
    try:
        if params.get("parallel", False):
            ...
            Tg_new, Tw_new, Ti_new = _timestep_numba_seq(
                Tg, Tw, Ti, _float(dt),
                _float(x[0]), _float(dx),
                _float(params["p"]), _float(params["m_dot"]), _float(params["Tin"]), _float(R), _float(params["Pr"]),
                _float(params["mu_g"]), _float(params["k_g"]),
