...
EPS_R = 1e-16        # small positive epsilon for resistances
...
class RuntimeTracker:
    ...
    def finalize(self, sim_t, steps, outdir: Path | None):
        ...
        # Persist trace CSV
        if outdir is not None and params.get("write_trace_csv", False) and self.records:
            import csv
...
def main(
    ...
    OUTDIR = None
    if save_results:
        OUTDIR = outdir if outdir is not None else _make_run_dir(prefix="runs")
        ...
        # Pure-API mode: keep logging quiet and avoid file I/O
        logging.getLogger().handlers.clear()
    ...
    Vw_cell = _float(np.pi*(r_w_o**2 - r_i**2)*dx)
    Vi_cell = _float(np.pi*(r_ins_o**2 - r_w_o**2)*dx)
    Cw_cell = _float(p["rho_w"]*Vw_cell*p["cp_w"])
    Ci_cell = _float(p["rho_i"]*Vi_cell*p["cp_i"])
    aw = _float(p["k_w"]/(p["rho_w"]*p["cp_w"]))
    ai = _float(p["k_i"]/(p["rho_i"]*p["cp_i"]))
    R_wall = _float(np.log(r_w_o/r_i)/(2*np.pi*p["k_w"]*dx))
    R_ins = _float(np.log(r_ins_o/r_w_o)/(2*np.pi*p["k_i"]*dx))
    R_Tw_to_Ti = _float(R_wall + R_ins)
    ...
    def compute_h_in_local(Tg_vec):
        ...
        Re = rho*u*Di/p["mu_g"]
        Re = np.maximum(Re, 1.0)
        Nu = 0.023*(Re**0.8)*(p["Pr"]**p["dittus_boelter_n"])
        return Nu*p["k_g"]/Di, u

    # --- initial conditions ---
    Tg = np.full(Nx, p["Tin"], dtype=DTYPE)
    Tw = np.full(Nx, p["T_init_wall"], dtype=DTYPE)
    Ti = np.full(Nx, p["T_init_ins"], dtype=DTYPE)
    h0, u0 = compute_h_in_local(np.full(Nx, p["Tin"], dtype=DTYPE))
    dt_cfl = p["CFL"]*dx/max(u0.max(), 1e-6)

    # --- Optional early-stop on gas outlet temperature ---
    reached_target = False
    stop_reason = "t_end"
    Tg_outlet_target = stop_at_Tg_outlet
    initial_Tg_outlet = float(Tg[-1])
    # Determine comparison direction if user didn't specify
    if Tg_outlet_target is not None:
        if stop_dir in ("le", "ge"):
            _cmp_dir = stop_dir
        else:
            _cmp_dir = "le" if Tg_outlet_target <= initial_Tg_outlet else "ge"
    else:
        _cmp_dir = None
    ...
    tau_g = ((p["p"]/(R*p["Tin"]))*A_flow*dx*p["cp_g"]) / ((h0*P_in).max()+1e-9)
    ...
    t = 0.0
    t_end = float(p["t_end"]) if max_sim_time is None else float(max_sim_time)
    tracker = RuntimeTracker(t_end, p.get("log_interval_s", 5.0), p.get("log_interval_steps", 1000))
    tracker.start()
    ...
    while t < t_end - 1e-12:
        ...
        if p["adv_scheme"] == "semi_lagrangian":
            ...
            dt_cfl_local = p["CFL"]*dx/np.maximum(u.max(), 1e-6)
        ...
        tau_i_local = Ci_cell/( p["h_out"]*P_out + 4*p["eps_rad"]*sigma*P_out*np.maximum(Ti,1)**3 + 1e-12 )
        dt_src_local = 0.35*min(np.min(tau_g_local), np.min(tau_w_local), np.min(tau_i_local))

        dt = float(max(1e-6, min(dt_cfl_local, dt_src_local, p["dt_max"])))
        ...
        try:
            if p.get("parallel", False):
                Tg_new, Tw_new, Ti_new = _timestep_numba(
                    ...
                    _float(p["p"]), _float(p["m_dot"]), _float(p["Tin"]), _float(R), _float(p["Pr"]),
                    _float(p["mu_g"]), _float(p["k_g"]),
                    _float(Di), _float(A_flow), _float(P_in), _float(P_out),
                    _float(p["cp_g"]), _float(Cw_cell), _float(Ci_cell), _float(R_Tw_to_Ti),
                    _float(p["h_out"]), _float(p["eps_rad"]), _float(sigma),
                    _float(p["theta_cond"]), _float(p["dittus_boelter_n"]), use_semi,
                    _float(p["Tamb"]),
            ...
            if p["adv_scheme"] == "semi_lagrangian":
                x_depart = x - u*dt
                Tg_adv = np.interp(x_depart, x, Tg, left=p["Tin"], right=Tg[-1]).astype(DTYPE, copy=False)
                q_gw = h_in*P_in*(Tw - Tg_adv)
                Cg_cell = (p["p"]/(R*Tg))*A_flow*dx*p["cp_g"]
                Tg_new = Tg_adv + dt * ( q_gw / np.maximum(Cg_cell,1e-12) )
            else:
                Tg_up = np.roll(Tg, 1); Tg_up[0] = p["Tin"]
                adv = -u*(Tg - Tg_up)/dx
            ...
            k_rad_vec = 4.0 * p["eps_rad"]*sigma*P_out * Ti3
            for i in range(Nx):
                a11 = Cw_cell/dt + h_in[i]*P_in + inv_R
                a12 = -inv_R
                a21 = -inv_R
                a22 = Ci_cell/dt + inv_R + p["h_out"]*P_out + k_rad_vec[i]
                rhs1 = (Cw_cell/dt)*Tw[i] + h_in[i]*P_in*Tg_new[i]
                rhs2 = (Ci_cell/dt)*Ti[i] + p["h_out"]*P_out*p["Tamb"] + p["eps_rad"]*sigma*P_out*((p["Tamb"]**4) - (Ti[i]**4)) + k_rad_vec[i]*Ti[i]
                det = a11*a22 - a12*a21
            ...
            if _a_w is not None and _a_i is not None:
                ...
                Tw_new = diffuse_axial_CN(Tw_new, aw, dt, dx, p["theta_cond"])
                Ti_new = diffuse_axial_CN(Ti_new, ai, dt, dx, p["theta_cond"])

        # Early-stop check on outlet temperature (uses Tg_new at x = L)
        if Tg_outlet_target is not None:
            Tg_out = float(Tg_new[-1])
            if (_cmp_dir == "le" and Tg_out <= Tg_outlet_target) or (_cmp_dir == "ge" and Tg_out >= Tg_outlet_target):
                reached_target = True
                stop_reason = f"Tg_outlet {'<=' if _cmp_dir == 'le' else '>='} {Tg_outlet_target} K"
